...
-- Scan visible terminal buffers (or a register, default '+') for file
-- references, fill the quickfix list, jump to the first.
function M.jump(opts)
  ...
  if opts.bang then
    ...
    for _, win in ipairs(api.nvim_tabpage_list_wins(0)) do
      ...
      if vim.bo[buf].buftype == 'terminal' then
        -- last 1000 lines: scrollback can be huge
        local lines = api.nvim_buf_get_lines(buf, -1001, -1, false)
        chunks[#chunks + 1] = table.concat(lines, '\n')
        local cwd = api.nvim_buf_get_name(buf):match('^term://(.-)//')
        if cwd and cwd ~= '' then
          bases[#bases + 1] = cwd
        end
      end
  ...
  bases[#bases + 1] = vim.fn.getcwd(-1, -1)
  for _, r in ipairs(M.roots()) do
    bases[#bases + 1] = r
  end
  if not text or text == '' then
    vim.notify('openloc: nothing to scan', vim.log.levels.WARN)
    return {}
  end
  local refs = detect.detect(text, bases)
  if #refs == 0 then
    vim.notify('openloc: no file references found', vim.log.levels.INFO)
    return refs
  end
  local items, seen = {}, {}
  for _, r in ipairs(refs) do
    local id = r.path .. ':' .. (r.line or 0) .. ':' .. (r.col or 0)
    if not seen[id] then
      seen[id] = true
      items[#items + 1] = { filename = r.path, lnum = r.line or 1, col = r.col or 1 }
    end
  end
  vim.fn.setqflist({}, ' ', { title = 'openloc', items = items })
  M.open(refs[1].path, refs[1].line, refs[1].col)
  return refs
end
